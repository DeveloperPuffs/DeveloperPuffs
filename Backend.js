...
import { deepFreeze } from "./Globals";
...
export const PROFILE_LOAD_ERROR = Symbol("PROFILE_LOAD_ERROR");
...
export async function loadProfile() {
        if (currentAccount === null) {
                throw new Error("Cannot load profile: User is not signed in to an account");
        }
...
export async function createProfile(username, avatarURL = null) {
        if (currentAccount === null) {
        ...
        if (profileCreationResult.error !== null) {
                console.error(`Failed to create profile ${profileCreationResult.status}: ${profileCreationResult.message}`);
                return false;
        }
